...
try {
  ...
  console.error("Please ensure FIREBASE_SERVICE_ACCOUNT_KEY is set in your .env file.\n");
  process.exit(1);
}
...
async function runStressTest() {
  ...
  for (let i = 0; i < NUM_VOTERS; i++) {
    ...
    for (const pos of positions) {
      ...
      if (posCands.length > 0) {
        const randomCand = posCands[Math.floor(Math.random() * posCands.length)];
  ...
  console.log(`Avg Throughput : ${(NUM_VOTERS / duration).toFixed(2)} votes/sec`);
  console.log(`=============================\n`);
}
